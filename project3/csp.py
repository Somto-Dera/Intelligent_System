...
# calculate chromatic number of csp problems
def chromaticNumber(csp):
    results, backtrack_num, time_duration = DFS(csp)
    chromaticNum = len(set(results.values()))
    return chromaticNum
...
# Depth first search
def DFS(csp, select_unassigned_variable=first_unassigned_variable,
                        order_domain_values=unordered_domain_values, inference=no_inference):
    start_time = time.perf_counter ()
    def backtrack(assignment, backtrack_num):
        if len(assignment) == len(csp.variables):
            return assignment, backtrack_num
        var = select_unassigned_variable(assignment, csp)
        for value in order_domain_values(var, assignment, csp):
            if 0 == csp.nconflicts(var, value, assignment):
                csp.assign(var, value, assignment)
                removals = csp.suppose(var, value)
                if inference(csp, var, value, assignment, removals):
                    result, backtrack_num = backtrack(assignment, backtrack_num)
                    if result is not None:
                        backtrack_num += 1
                        return result, backtrack_num
                csp.restore(removals)
        csp.unassign(var, assignment)
        return None
    
    result, backtrack_num = backtrack({}, 0)
    assert result is None or csp.goal_test(result)
    end_time = time.perf_counter ()
    time_duration = end_time - start_time
    return result, backtrack_num, time_duration
